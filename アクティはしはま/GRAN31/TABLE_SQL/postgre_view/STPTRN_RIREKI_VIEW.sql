-- *************************************************
-- ***   Create view STPTRN_RIREKI_VIEW
-- *************************************************
   DROP VIEW IF EXISTS STPTRN_RIREKI_VIEW;
CREATE VIEW STPTRN_RIREKI_VIEW AS 
    SELECT DENDT,
           DENNO,
           SCLMANNO,
           SCLKBN,
           ...
           RSTSFLG
      FROM (
         SELECT ROW_NUMBER() OVER (PARTITION BY SCLMANNO, COALESCE(STSFLG, '0'), STPDT, COALESCE(RSTSFLG, '0'), RETNKDT ORDER BY DENDT || TO_CHAR(DENNO, '0000') desc) RNO, 
                 DENDT,
                 DENNO,
                 SCLMANNO,
                 SCLKBN,
