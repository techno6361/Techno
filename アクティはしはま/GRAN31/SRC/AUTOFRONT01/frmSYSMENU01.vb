...
#Region "▼イベント定義"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub btnCM_Click(sender As System.Object, e As System.EventArgs) Handles btnCM.Click
        Try
            Using frm As New frmCM
                frm.CM = _dcCM
                frm.ShowDialog()
                _dcCM = frm.CM
            End Using
...
#Region "▼関数定義"
    ...
    Private Sub Init()
        Try
            ...
            Me.cmbCMPort.SelectedIndex = tmp6
            ' *** END ADD 2018/09/26 TERAYAMA リスト表示バグ修正
            ...
            For Each n In doc("configuration")("appSettings")
                If n.Name = "add" Then
                    ...
                    Select Case n.Attributes.GetNamedItem("key").Value
                        ...
                        Case "CM_COM"
                            'コインメックポート
                            Me.cmbCMPort.Text = n.Attributes.GetNamedItem("value").Value
                        Case "SK121_COM"
                            'レシートプリンタポート
            ...
            For Each n In doc("configuration")("appSettings")
                If n.Name = "add" Then
                    ...
                    Select Case n.Attributes.GetNamedItem("key").Value
                        ...
                        Case "CM_COM"
                            'コインメックポート
                            n.Attributes.GetNamedItem("value").Value = Me.cmbCMPort.Text
                        Case "SK121_COM"
                            'ビルバリポート
    ...
    Private Sub CheckCM()
        Try
            ...
            Me.btnCM.BackColor = Color.ForestGreen

            '【異常】

            '代表異常
            If _dcCM.CmAbnormal1.Substring(7, 1).Equals("1") Then
                Me.btnCM.BackColor = Color.Red
            End If
            '動作可
