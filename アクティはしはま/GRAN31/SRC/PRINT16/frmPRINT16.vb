...
#Region "▼宣言部"
    ...
    ' OPEN/SAVEファイル
    Const REPORT_NAME = "入場履歴一覧"

    ' フォーム名
    Const FORM_NAME = "入場履歴一覧"
...
#Region "▼イベント定義"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmPRINT16_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '画面初期設定
            Init()
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmPRINT16_Shown(sender As System.Object, e As System.EventArgs) Handles MyBase.Shown
        Try
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmPRINT16_FormClosed(sender As System.Object, e As System.Windows.Forms.FormClosedEventArgs) Handles MyBase.FormClosed
        Try
            If Not _kbmastDt Is Nothing Then _kbmastDt.Dispose()
            If Not _eigmtbDt Is Nothing Then _eigmtbDt.Dispose()
    ...
    Private Sub btnFIND_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFIND.Click
        Try
            ...
            If Not GetENTTRA(date1, date2) Then
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmPRINT16_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
...
#Region "▼関数定義"
    ...
    Private Function PrintList(ByVal list As BindingList(Of ENTTRA_View)) As Boolean
        ...
        Try
            ...
            ' 打ち放題
            excel.Cells(6, 5) = "【打放60分】 " & Me.lblJKN60.Text & "  【打放120分】 " & Me.lblJKN120.Text & "  【打放180分】 " & Me.lblJKN180.Text
            ...
            For Each day_rows In group_list
                ...
                For Each row In day_rows
                    ...
                    '受付内容
                    excel.Cells(row_index, 4) = row.ENTNM
                    ...
                    ' 入場料
                    excel.Cells(row_index, 7) = row.ENTKN

                    ' ポイント
                    excel.Cells(row_index, 8) = row.POINT
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetENTTRA(ByVal date1 As String, ByVal date2 As String) As Boolean
        ...
        Try
            ...
            sql.Append(String.Format(" AND A.ENTDT >= '{0}'", date1))
            sql.Append(String.Format(" AND A.ENTDT <= '{0}'", date2))
            'プレイ区分
            If rdoEIGKB1.Checked Then
                sql.Append(" AND EIGKB = '1'")
            ElseIf rdoEIGKB2.Checked Then
            ...
            Dim query = From x In dt.AsEnumerable
                        Select New ENTTRA_View With {
                            ...
                            .ENTKN = CType(x("ENTKN").ToString, Integer),
                            .POINT = CType(x("SRTPO").ToString, Integer)
                          }
            ...
            For Each row In query
                ...
                If row.EIGKB.Equals("1") Then
                    Dim kbmast = _kbmastDt.AsEnumerable.Where(Function(x) x("NKBNO").ToString = row.KSBKB.ToString).First()
                    data.ENTNM = kbmast("CKBNAME").ToString
                Else
                    Dim eigmtb = _eigmtbDt.AsEnumerable.Where(Function(x) x("RKNKB").ToString = row.RKNKB.ToString And x("JKNKB").ToString = row.KSBKB.ToString).First()
                    data.ENTNM = "打ち放題" & eigmtb("JKNMM").ToString & "分"
                    Select Case eigmtb("JKNMM").ToString
                        Case "60"
                            Me.lblJKN60.Text = (CType(Me.lblJKN60.Text, Integer) + 1).ToString("#,##0")
                        Case "120"
                            Me.lblJKN120.Text = (CType(Me.lblJKN120.Text, Integer) + 1).ToString("#,##0")
                        Case "180"
                            Me.lblJKN180.Text = (CType(Me.lblJKN180.Text, Integer) + 1).ToString("#,##0")
                    End Select
                End If

                data.MANNO = row.MANNO
                data.CCSNAME = row.CCSNAME
                data.ENTKN = row.ENTKN
