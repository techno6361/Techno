...
#Region "▼イベント定義"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub dgvNKNTRN_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dgvNKNTRN.Click
        Try
            If Me.dgvNKNTRN.RowCount = 0 Then
                Exit Sub
            End If

            Dim intRow As Integer

            If Me.dgvNKNTRN.Rows.Count <> 0 Then
                intRow = Me.dgvNKNTRN.CurrentRow.Index

                _intNCSNO = CType(Me.dgvNKNTRN.getValue("MANNO", intRow).ToString, Integer)
            End If
...
#Region "▼関数定義"
    ...
    Private Function PrintList(ByVal list As BindingList(Of NKNTRN_View)) As Boolean
        ...
        Try
            ...
            ' 出力日
            excel.Cells(1, 14) = String.Format("出力日：{0}", DateTime.Now.ToString("yyyy/MM/dd"))
            ...
            For Each day_rows In group_list
                ...
                For Each row In day_rows
                    ...
                    ' 入金額
                    excel.Cells(row_index, 7) = row.NKNKN.ToString("N0")

                    ' プレミアム
                    excel.Cells(row_index, 8) = row.PREMKN.ToString("N0")

                    ' ポイント
                    excel.Cells(row_index, 9) = row.POINT.ToString("N0")

                    ' 担当者
                    excel.Cells(row_index, 10) = row.STFNAME

                    ' 投入金額
                    excel.Cells(row_index, 11) = row.NYUKN.ToString("N0")

                    ' おつり
                    excel.Cells(row_index, 12) = row.TURIKN.ToString("N0")

                    ' 支払い区分
                    excel.Cells(row_index, 13) = row.CPAYKBN

                    ' 入金前
                    excel.Cells(row_index, 14) = row.ZANAKN.ToString("N0")

                    ' 入金後
                    excel.Cells(row_index, 15) = row.ZANBKN.ToString("N0")
    ...
    Private Function GetNKNTRN(ByVal date1 As String, ByVal date2 As String, ByVal stsflg As String) As Boolean
        ...
        Try
            ...
            sql.Append(" SELECT ")
            sql.Append(" A.*")
            sql.Append(",CASE WHEN A.PRERT IS NULL THEN 0 ELSE A.PRERT END AS BIRTHSVC")
            ...
            sql.Append(" AND (A.STSFLG = '0' OR A.STSFLG = '1' OR A.STSFLG = '2' OR A.STSFLG = '9')")
            sql.Append(String.Format(" AND A.DENDT >= '{0}'", date1))
            sql.Append(String.Format(" AND A.DENDT <= '{0}'", date2))
            sql.Append(" ORDER BY A.DENDT DESC,A.DENNO DESC")
            ...
            sql.Append(" ORDER BY UDNDT DESC,UDNNO DESC")
            ...
            Dim join_query = From n In nkntrn_query
                             ...
                             Select New With {
                                 ...
                                 .INSDTM2 = n.INSDTM.ToString("HH:mm:ss"),
                                 .STSFLG = n.STSFLG,
                                 .MANNO = n.MANNO,
                                 .CCSNAME = n.CCSNAME,
                                 .NKNKN = n.NKNKN,
...
'    Public Border As Excel.Border = Nothing
'    Public BorderStartRangeString As String
'    Public BorderEndRangeString As String

'    Public Sub New()
'    End Sub

'    Public Sub New(ByVal _sheet As Excel.Worksheet)
'        Sheet = _sheet
'    End Sub

'    Public Sub New(ByRef _sheet As Excel.Worksheet, ByVal _strStaRange As String, ByVal _strEndRange As String)
'        Sheet = _sheet
'        BorderStartRangeString = _strStaRange
'        BorderEndRangeString = _strEndRange
'    End Sub
...
'    ''' <remarks></remarks>
'    Public Sub HPageBreaks(ByVal row_index As Integer)
'        Sheet.HPageBreaks.Add(Sheet.Range(String.Format("A{0}", row_index)))
'    End Sub
...
'                         Optional ByVal weight As Excel.XlBorderWeight = Excel.XlBorderWeight.xlThin,
'                         Optional ByVal index As Excel.XlBordersIndex = Excel.XlBordersIndex.xlEdgeTop)
'        Dim staRange = BorderStartRangeString & (row_index).ToString
'        Dim endRange = BorderEndRangeString & (row_index).ToString
'        Border = Sheet.Range(staRange, endRange).Borders(index)
...
'    ''' <param name="row_index"></param>
'    ''' <remarks></remarks>
'    Public Sub DrawDoubleLine(ByVal row_index As Integer)
'        DrawLine(row_index, Excel.XlLineStyle.xlDouble, Excel.XlBorderWeight.xlThick)
'    End Sub
'End Class
