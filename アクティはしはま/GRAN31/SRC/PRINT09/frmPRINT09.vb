...
#Region "▼宣言部"
    ...
    ' OPEN/SAVEファイル
    Const REPORT_NAME = "カード支出一覧"

    ' フォーム名
    Const FORM_NAME = "カード支出一覧"
...
#Region "▼イベント定義"
    ...
    Private Sub btnFIND_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFIND.Click
        Try
            ...
            If Not GetNKNTRN(date1, date2) Then
...
#Region "▼関数定義"
        ...
        Try
            ...
            For Each day_rows In group_list
                ...
                For Each row In day_rows
                    ...
                    ' 支出内容
                    excel.Cells(row_index, 5) = row.NKNNM

                    ' 支出額
                    excel.Cells(row_index, 6) = row.NKNKN.ToString("N0")

                    ' 内消費税
                    excel.Cells(row_index, 7) = row.NZEIKN.ToString("N0")
                ...
                excel.Cells(row_index, 7) = day_rows.Select(Function(x) x.NZEIKN).Sum().ToString("N0")
                excel.Cells(row_index, 8) = day_rows.Select(Function(x) x.POINT).Sum().ToString("N0")
                excel.Cells(row_index, 10) = day_rows.Select(Function(x) x.ZANAKN).Sum().ToString("N0")
                excel.Cells(row_index, 11) = day_rows.Select(Function(x) x.ZANBKN).Sum().ToString("N0")
            ...
            excel.Cells(row_index, 7) = list.Select(Function(x) x.NZEIKN).Sum().ToString("N0")
            excel.Cells(row_index, 8) = list.Select(Function(x) x.POINT).Sum().ToString("N0")
            excel.Cells(row_index, 10) = list.Select(Function(x) x.ZANAKN).Sum().ToString("N0")
            excel.Cells(row_index, 11) = list.Select(Function(x) x.ZANBKN).Sum().ToString("N0")
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetNKNTRN(ByVal date1 As String, ByVal date2 As String) As Boolean
        ...
        Try
            ...
            sql.Append(" WHERE A.DATKB = '1' AND B.DATKB = '1' AND A.STSFLG = '8'")
            ...
            Dim query = From x In dt.AsEnumerable
                        Select New NKNTRN_View With {
                            ...
                            .NKNNM = x("NKNNM").ToString,
                            .NKNKN = CInt(x("NKNKN")),
                            .NZEIKN = CInt(x("NZEIKN")),
                            .POINT = CInt(x("POINT")),
            ...
            For Each row In query
                ...
                data.NKNNM = row.NKNNM
                data.NKNKN = row.NKNKN
                data.NZEIKN = row.NZEIKN
